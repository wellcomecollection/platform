...
def event(critical_hook, alarm_name, alarm_reason):
    ...
    dimensions = [
        {"name": "TargetGroup", "value": "targetgroup/api/e0aa3403356f01e9"},
        {"name": "LoadBalancer", "value": "app/api/e87a4a5f32874d8b"},
    ]
...
class TestPrepareSlackPayload:
    ...
    @pytest.mark.skip(reason="filter_log_events() isn't implemented in moto")
    @moto.mock_logs
    def test_including_cloudwatch_messages(self, sess):
        ...
        client.create_log_stream(
            logGroupName="platform/loris", logStreamName="logstream001"
        )

        def _event(minute, status_code):
            return {
                "timestamp": datetime_to_cloudwatch_ts(
