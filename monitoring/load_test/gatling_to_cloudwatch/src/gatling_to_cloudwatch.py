...
def _send_metric(client, namespace, metric_data):
    ...
    return client.put_metric_data(Namespace=namespace, MetricData=metric_data)
...
def send_assertions_to_cloudwatch(client, event):
    ...
    metric_data = [_generate_metric(a, timestamp) for a in assertions]

    resp = _send_metric(client=client, namespace=namespace, metric_data=metric_data)
    print(f"resp = {resp}")
