  ...
  def apply(work: IdentifiedWork, includes: WorksIncludes): DisplayWorkV1 = {
    ...
    DisplayWorkV1(
      ...
      creators = work.contributors.map {
        contributor: Contributor[Displayable[AbstractAgent]] => DisplayAbstractAgent(contributor.agent)
      },
      subjects = work.subjects.flatMap { subject => subject.concepts.map { DisplayConcept(_) } },
      genres = work.genres.flatMap { genre => genre.concepts.map { DisplayConcept(_) } },
      identifiers =
