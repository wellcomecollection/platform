...
object StorageStrategy {
  ...
  implicit def typeStorageStrategy[T](
    implicit
      jsonStorageStrategy: StorageStrategy[Json],
      encoder: Encoder[T],
      decoder: Decoder[T]
   ) = new StorageStrategy[T] {
    def store(t: T): StorageStream = jsonStorageStrategy.store(t.asJson)
    def retrieve(input: InputStream) = jsonStorageStrategy.retrieve(input).flatMap(_.as[T].toTry)

  }

  implicit def streamStorageStrategy: StorageStrategy[InputStream] = new StorageStrategy[InputStream] {
  ...
  implicit def jsonStorageStrategy: StorageStrategy[Json] = new StorageStrategy[Json] {
  ...
  implicit def stringStorageStrategy: StorageStrategy[String] = new StorageStrategy[String] {
