...
class VersionUpdaterTest extends FunSpec with Matchers {
  ...
  it("creates a VersionUpdater instance for an hlist"){
    ...
    val version = 1
    val originalTestVersion = TestVersion("something", version, "somethingElse")
    val newVersion = 3
    versionUpdater.updateVersion(gen.to(originalTestVersion), newVersion) shouldBe gen.to(originalTestVersion.copy(version = newVersion))
  }
  ...
  it("creates a VersionUpdater instance for a case class"){
    ...
    val originalTestVersion = TestVersion("something", version, "somethingElse")
    val newVersion = 3
    versionUpdater.updateVersion(originalTestVersion, newVersion) shouldBe originalTestVersion.copy(version = newVersion)
  }
}
