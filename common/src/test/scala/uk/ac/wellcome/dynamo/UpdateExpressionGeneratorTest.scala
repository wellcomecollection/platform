...
class UpdateExpressionGeneratorTest extends FunSpec with Matchers {
  ...
  it("generates an update expression for a case class") {
    val example = Example(id = "111", data = "something", number = 1, date = Instant.now())

    val expectedUpdateExpression = set('data -> example.data) and set('number -> example.number) and set('date -> example.date)

    val updateExpressionGenerator = UpdateExpressionGenerator[Example]

    updateExpressionGenerator.generateUpdateExpression(example) shouldBe Some(expectedUpdateExpression)
  }
