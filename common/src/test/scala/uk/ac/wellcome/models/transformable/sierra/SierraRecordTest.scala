...
class SierraRecordTest extends FunSpec with Matchers {
  ...
  describe("toItemRecord") {
    it("return a successful try for valid item json") {
      ...
      val sierraRecord = SierraRecord(id = id, data = data, modifiedDate = modifiedDate)

      sierraRecord.toItemRecord shouldBe Success(SierraItemRecord(id = id, data = data, modifiedDate = modifiedDate, bibIds = List(bibId)))
    }

    it("return a failure for an invalid json") {
      val sierraRecord = SierraRecord(id = "i12345", data = "not a json string", modifiedDate = modifiedDate)
    ...
    it("return a failure if the json is valid but contains no bibIds") {
      val sierraRecord = SierraRecord(id = "i12345", data = "{}", modifiedDate = modifiedDate)
    ...
    it("return a failure if bibIds is a list of non strings") {
      val sierraRecord = SierraRecord(id = "i12345", data = """{"bibIds":[1,2,3]}""", modifiedDate = modifiedDate)
    ...
    it("return a failure if bibIds is not a list") {
      val sierraRecord = SierraRecord(id = "i12345", data = """{"bibIds":"blah"}""", modifiedDate = modifiedDate)
