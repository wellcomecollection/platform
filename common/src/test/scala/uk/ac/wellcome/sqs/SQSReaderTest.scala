...
class SQSReaderTest extends FunSpec with Matchers with ScalaFutures with IntegrationPatience with SQSLocal{
  ...
  it("should get messages from the SQS queue, limited by the maximum number of messages and return them"){
    ...
    val sqsReader = new SQSReader(sqsClient, sqsConfig, waitTime = 20 seconds, 2)
