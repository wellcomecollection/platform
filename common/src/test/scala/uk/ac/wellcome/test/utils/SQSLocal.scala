...
trait SQSLocal extends Suite with BeforeAndAfterEach{
  ...
  val sqsClient = AmazonSQSClientBuilder.standard()
    ...
    .withEndpointConfiguration(new EndpointConfiguration(s"http://localhost:9324", "localhost")).build()
  ...
  override def beforeEach(): Unit = {
    super.beforeEach()
    sqsClient.purgeQueue(new PurgeQueueRequest().withQueueUrl(idMinterQueueUrl))
  }
}
