...
object HybridRecordEnricher {
  ...
  def create[T, O](f: (T, Int, String) => O) = new HybridRecordEnricher[T] {
    type Out = O
    override def enrichedHybridRecordHList(record: T, version: Int)(
      s3key: String): Out = f(record, version, s3key)
  }
  ...
  implicit def enricher[T <: Id,
    ...
    annotations: Annotations.Aux[CopyToDynamo, T, A],
    zipper: Zip.Aux[R :: A :: HNil, D],
    collector: Collect.Aux[D, CollectorPoly.type, E],
    prepend: Prepend[F, E]) = create {
