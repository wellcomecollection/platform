...
case class DynamoConfig(region: String, applicationName: String, arn: String, table: String)
...
object DynamoConfigModule extends TwitterModule {
  ...
  private val table = flag[String]("aws.dynamo.tableName", "", "Name of the DynamoDB table")

  @Singleton
  @Provides
  def providesDynamoConfig(): DynamoConfig = DynamoConfig(
    region(), applicationName(), arn(), table())
}
