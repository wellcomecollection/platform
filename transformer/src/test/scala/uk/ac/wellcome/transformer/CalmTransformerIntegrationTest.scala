...
class CalmTransformerIntegrationTest
  ...
  test("it should poll the dynamo stream for calm data, transform it into unified items and push them into the id_minter SNS topic") {
    Scanamo.put(dynamoDbClient)(calmDataTableName)(
      CalmTransformable("RecordID1", "Collection", "AltRefNo1", "RefNo1", """{"AccessStatus": ["public"]}"""))
    ...
    Scanamo.put(dynamoDbClient)(calmDataTableName)(
      CalmTransformable("RecordID2", "Collection", "AltRefNo2", "RefNo2", """{"AccessStatus": ["secret"]}"""))
    ...
    eventually {
      ...
      assertSNSMessageContainsCalmDataWith(snsMessages.head, Some("public"))
      assertSNSMessageContainsCalmDataWith(snsMessages.tail.head, Some("secret"))
    }
  }

  private def assertSNSMessageContainsCalmDataWith(snsMessage: MessageInfo, AccessStatus: Option[String]): Any = {
    //currently for calm data we only output hardcoded sample values
    snsMessage.message shouldBe JsonUtil.toJson(
      UnifiedItem(
