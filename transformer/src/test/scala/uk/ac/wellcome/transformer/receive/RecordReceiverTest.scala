...
class RecordReceiverTest extends FunSpec with MockitoSugar with ScalaFutures with Matchers with IntegrationPatience{
  ...
  it("should return a failed future if it's unable to parse the dynamo record") {
    val recordReceiver = new RecordReceiver(config, mockSNS)
    val future = recordReceiver.receiveRecord(new RecordAdapter(createInvalidRecord))
  ...
  it("should return a failed future if it's unable to transform the parsed record") {
    val recordReceiver = new RecordReceiver(config, mockSNS)
    val future = recordReceiver.receiveRecord(new RecordAdapter(createNonTransformableRecord))
  ...
  private def mockFailPublishMessage = {
    val mockSNS = mock[AmazonSNS]
    when(mockSNS.publish(any[PublishRequest]())).thenThrow(new RuntimeException("Failed publishing message"))
    mockSNS
  }
  ...
  private def createMockRecord = {
    val record = new Record()
    val streamRecord = createStreamRecord("""{"foo": ["bar"], "AccessStatus": ["TopSekrit"]}""")
    record.withDynamodb(streamRecord)
    record
  }
  ...
  private def createInvalidRecord = {
    val record = new Record()
    val streamRecord = new StreamRecord()
    streamRecord.addNewImageEntry("something", new AttributeValue("something-else"))
    record.withDynamodb(streamRecord)
    record
  }
}
