...
class ZippedBagFileTest extends FunSpec with Matchers with ZipBagItFixture with FileGenerators {

  describe("locateBagInfo") {
    it("locates a bag-info file in a zipped bag") {
      withBagItZip() { zippedBag =>
        ZippedBagFile.locateBagInfo(new ZipFile(zippedBag)) shouldBe Success("bag-info.txt")
      }
    }

    it("locates a valid bag-info file in a zip file") {
      val validBagInfoFiles = Table("filename",
        "bag-info.txt",
        "bag/bag-info.txt",
      ...
      forAll(validBagInfoFiles) { bagInfofilename =>
        withZipFile(List(createFileEntry(bagInfofilename))) { zippedBag =>
          ZippedBagFile.locateBagInfo(new ZipFile(zippedBag)) shouldBe Success(bagInfofilename)
        }
      }
    }

    it("fails if there is no valid bag-info file in zip file") {
      val invalidBagInfoFiles = Table("filename",
        "",
        "no-bag.txt",
        "Xbag-info.txt",
        "bag-info.txtX",
      ...
      forAll(invalidBagInfoFiles) { invalidBagInfoFilename =>
        withZipFile(List(createFileEntry(invalidBagInfoFilename))) { zippedBag =>
          ZippedBagFile.locateBagInfo(new ZipFile(zippedBag)).failure.exception.getMessage shouldBe
  ...
  describe("bagPathFromBagInfoPath") {
    ...
    it("finds the bag path when in a parent directory") {
      ZippedBagFile.bagPathFromBagInfoPath("bag/bag-info.txt") shouldBe Some("bag/")
    }

    it("ignores bag-info.txt not at the end") {
      ZippedBagFile.bagPathFromBagInfoPath("bag-info.txt/bag/bag-info.txt") shouldBe Some("bag-info.txt/bag/")
    }
