      ...
      withMaterializer { implicit materializer =>
        withBagItZip(dataFileCount = 2) { file =>
          ...
          whenReady(futureResult) { result =>
            ...
            getContentFromS3(
              bucket,
              s"${archiveJob.bagLocation.completeFilepath}/tagmanifest-sha256.txt") shouldBe expectedTagManifestStream
          }
        }
      }
    }
              ...
              inside(errors.head) {
                case UploadError(location, exception, t) =>
                  ...
                  location shouldBe ObjectLocation(
                    "not-a-valid-bucket",
                    s"${archiveJob.bagLocation.completeFilepath}/tagmanifest-sha256.txt")
              }
          }
        }
      }
