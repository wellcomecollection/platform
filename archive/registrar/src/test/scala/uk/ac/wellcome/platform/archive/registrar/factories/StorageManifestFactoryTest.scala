...
class StorageManifestFactoryTest
  ...
  it(
    "returns a left of registrar error if there are no files at the specified bag location") {
    val (callbackUrl, requestId) = createCallbackUrl
  ...
  it(
    "returns a left of registrar error if the bagLocation has an invalid manifestfile") {
    val (callbackUrl, requestId) = createCallbackUrl
