...
import uk.ac.wellcome.platform.archive.archivist.models.errors.{ArchiveJobError, ChecksumNotMatchedOnUploadError, FileNotFoundError, InvalidBagManifestError}
...
class ArchiveZipFileFlowTest
            ...
            withBagItZip() {
              case (bagName, zipFile) =>
                ...
                whenReady(verification) { result =>
                  ...
                  result shouldBe List(Right(ArchiveComplete(ingestContext.archiveRequestId, BagLocation(storageBucket.name, "archive", BagPath(s"$DigitisedStorageType/$bagName")), None)))
  ...
  it("notifies failure when verifying and uploading a bag with no bag-info.txt file") {
    withLocalS3Bucket { storageBucket =>
      withActorSystem { actorSystem =>
        withMaterializer(actorSystem) { implicit materializer =>
          withLocalSnsTopic { reportingTopic =>
            ...
            withBagItZip(createBagInfoFile = _ => None) {
              case (_, zipFile) =>
                ...
                whenReady(verification) { result =>
                  inside(result.toList) { case List(Left(FileNotFoundError(path, ingestRequest))) =>
  ...
  it("notifies failure when verifying and uploading a bag with invalid data manifest file") {
    withLocalS3Bucket { storageBucket =>
      withActorSystem { actorSystem =>
        withMaterializer(actorSystem) { implicit materializer =>
          withLocalSnsTopic { reportingTopic =>
            val bagUploaderConfig = createBagUploaderConfig(storageBucket)
            withBagItZip(createDataManifest = _ => Some(FileEntry("manifest-sha256.txt","dgfssjhdfg"))) {
              case (bagName, zipFile) =>
                val uploader = ArchiveZipFileFlow(
                  bagUploaderConfig,
                  SNSConfig(reportingTopic.arn))
                ...
                whenReady(verification) { result =>
                  inside(result.toList) { case List(Left(InvalidBagManifestError(archiveJob, "manifest-sha256.txt"))) =>
                    archiveJob.bagLocation shouldBe BagLocation(storageBucket.name, "archive", BagPath(s"$DigitisedStorageType/$bagName"))
                  }
