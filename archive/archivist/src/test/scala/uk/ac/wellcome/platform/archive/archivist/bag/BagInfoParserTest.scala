...
class BagInfoParserTest extends FunSpec with BagIt with Matchers with IngestRequestContextGenerators with RandomThings{
  ...
  it("extracts a BagInfo object from a valid bagInfo file") {
    ...
    val bagInfoString = bagInfoFileContents(externalIdentifier, sourceOrganisation, payloadOxum, baggingDate)

    BagInfoParser.parseBagInfo(request,IOUtils.toInputStream(bagInfoString, "UTF-8")) shouldBe Right(BagInfo(externalIdentifier, sourceOrganisation, payloadOxum, baggingDate))
  }
  ...
  it("returns a left of invalid bag info error if there is no bagging date in bag-info.txt"){
    val bagInfoString =
      s"""|External-Identifier: $randomExternalIdentifier
          |Source-Organization: $randomSourceOrganisation
          |Payload-Oxum: ${randomPayloadOxum.payloadBytes}.${randomPayloadOxum.numberOfPayloadFiles}""".stripMargin

    BagInfoParser.parseBagInfo(request,IOUtils.toInputStream(bagInfoString, "UTF-8")) shouldBe Left(InvalidBagInfo(request, List("Bagging-Date")))
  }

  it("returns a left of invalid bag info error if the bagging date is invalid in bag-info.txt"){
    val bagInfoString =
      s"""|External-Identifier: $randomExternalIdentifier
          |Source-Organization: $randomSourceOrganisation
          |Payload-Oxum: ${randomPayloadOxum.payloadBytes}.${randomPayloadOxum.numberOfPayloadFiles}
  ...
  it("returns a left of invalid bag info error if bag-info.txt is empty"){
    ...
    BagInfoParser.parseBagInfo(request,IOUtils.toInputStream(bagInfoString, "UTF-8")) shouldBe Left(InvalidBagInfo(request, List("External-Identifier","Source-Organization", "Payload-Oxum", "Bagging-Date")))
  }
