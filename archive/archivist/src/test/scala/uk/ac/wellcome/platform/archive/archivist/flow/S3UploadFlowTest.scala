...
class S3UploadFlowTest extends FunSpec with S3 with ScalaFutures with Akka with IntegrationPatience{
  ...
  it("uploads a big file of random bytes") {
    withActorSystem { implicit actorSystem =>
      withMaterializer(actorSystem) { implicit materializer =>
        withLocalS3Bucket { bucket =>
          val res = Array.fill(7 * 1024 * 1024)((scala.util.Random.nextInt(256) - 128).toByte)
          val s3Key = "key.txt"
          val futureResult = Source.single(ByteString(res))
            .via(S3UploadFlow(ObjectLocation(bucket.name, s3Key))(s3Client)).runWith(Sink.seq)
    ...
    withActorSystem { implicit actorSystem =>
      withMaterializer(actorSystem) { implicit materializer =>
        withLocalS3Bucket { bucket =>
          ...
          val futureResult = Source.combine(Source.single(ByteString("abcdesf", "UTF-8")), Source.failed(expectedException))(Concat(_))
          .via(S3UploadFlow(ObjectLocation(bucket.name, s3Key))(s3Client)).runWith(Sink.seq)
          ...
          whenReady(futureResult.failed) { result =>
            ...
            val multipartUploadListing = s3Client.listMultipartUploads(new ListMultipartUploadsRequest(bucket.name))
    ...
    implicit val materializer = ActorMaterializer(
      ActorMaterializerSettings(actorSystem).withSupervisionStrategy(decider)
        ...
        withLocalS3Bucket { bucket =>
          val s3Key = "key.txt"
          val expectedException = new RuntimeException("BOOM!")
          val futureResult = Source.combine(Source.single(ByteString("abcdesf", "UTF-8")), Source.failed(expectedException), Source.single(ByteString("abcdesf", "UTF-8")))(Concat(_))
            .via(S3UploadFlow(ObjectLocation(bucket.name, s3Key))(s3Client)).runWith(Sink.seq)
  ...
  it("respects the supervision strategy on internal failure"){
    ...
    implicit val materializer = ActorMaterializer(
      ActorMaterializerSettings(actorSystem).withSupervisionStrategy(decider)
          ...
          val s3Key = "key.txt"
          val futureResult = Source.single(ByteString("abcdesf", "UTF-8"))
