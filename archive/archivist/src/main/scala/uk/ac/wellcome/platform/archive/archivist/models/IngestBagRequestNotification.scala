...
case class IngestBagRequestNotification(archiveRequestId: UUID,
...
object IngestBagRequestNotification {
  ...
  implicit val uuidDecoder: Decoder[UUID] = Decoder.instance { cursor =>
    cursor.as[String] match {
      case Right(str) => try Right(UUID.fromString(str)) catch {
        case _: IllegalArgumentException => Left(DecodingFailure("UUID", cursor.history))
      }
      case l @ Left(_) => l.asInstanceOf[Decoder.Result[UUID]]
    }
  }

  implicit val ingestBagRequestNotificationDecoder: Decoder[IngestBagRequestNotification] = deriveDecoder
  implicit val ingestBagRequestNotificationEncoder: Encoder[IngestBagRequestNotification] = deriveEncoder
}