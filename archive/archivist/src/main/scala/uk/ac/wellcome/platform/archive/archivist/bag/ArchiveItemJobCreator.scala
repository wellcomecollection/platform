...
object ArchiveItemJobCreator extends Logging {
  ...
  def createArchiveItemJobs(job: ArchiveJob, delimiter: String): Either[ArchiveJob, List[ArchiveItemJob]] = {
    val zipLocations = job.bagManifestLocations.map(manifestLocation => ZipLocation(job.zipFile, manifestLocation.toObjectLocation))
    val triedArchiveItemJobLists: Try[List[List[ArchiveItemJob]]] = zipLocations.traverse { zipLocation =>
