...
object FoldEitherFlow {
  def apply[L, R, Out](default: L => Out)(flow: Flow[R, Out, NotUsed]): Flow[Either[L,R], Out, NotUsed] = {
    Flow.fromGraph(GraphDSL.create(Broadcast[Either[L,R]](2),Merge[Out](2))(Keep.none){implicit builder =>
      ...
      (broadcast, merge) => {
        ...
        broadcast.out(1).collect{case Left(something) => default(something)} ~> merge.in(1)
