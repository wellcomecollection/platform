...
class Archivist(
  ...
  def run(): Future[Done] = {
    ...
    val unitFlow = Flow[ArchiveError[IngestBagRequest]].map(_ => ())
    ...
    val errorOrSucceedFlow =
      FoldEitherFlow[IngestError, FileDownloadComplete, Unit](
        ifLeft = unitFlow
      )(
        ifRight = archiveAndNotifyFlow
      )
