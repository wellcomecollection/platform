...
import uk.ac.wellcome.platform.archive.archivist.models.{BagItConfig, BagUploaderConfig, UploadConfig}
  ...
  def buildBagUploaderConfig(config: Config): BagUploaderConfig = {
    BagUploaderConfig(
      uploadConfig = UploadConfig(
        ...
        uploadPrefix = config.getOrElse[String]("upload.prefix")(default = "archive")
      ),
      bagItConfig = BagItConfig(
        digestDelimiterRegexp = config.getOrElse[String]("digest.delimiterRegexp")(default = " +")
      ),
