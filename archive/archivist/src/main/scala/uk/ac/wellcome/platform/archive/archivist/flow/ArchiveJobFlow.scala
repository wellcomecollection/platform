...
object ArchiveJobFlow extends Logging {
  def apply(delimiter: String, parallelism: Int, ingestBagRequest: IngestBagRequest)(implicit s3Client: AmazonS3)
  ...
  private def mapReduceArchiveItemJobs(delimiter: String, parallelism: Int, ingestBagRequest: IngestBagRequest)(
    implicit s3Client: AmazonS3): Flow[List[ArchiveItemJob], Either[ArchiveJobError,ArchiveComplete], NotUsed] =
    ...
    .map {
      case (Nil, archiveJob) => Right(ArchiveComplete(
