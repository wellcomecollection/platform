...
def test_looks_up_bag(dynamodb_resource, s3_client, bucket_bag, table_name_bag):
    ...
    given_bag_in_vhs(
        bag_id, stored_bag, dynamodb_resource, s3_client, bucket_bag, table_name_bag
    )

    response = fetch_bag(
        dynamodb_resource, table_name_bag, s3_client, bucket_bag, bag_id
    )
...
def given_bag_in_vhs(
    ...
    table.put_item(
        Item={"id": bag_id, "location": {"key": key, "namespace": bucket_bag}}
    )
