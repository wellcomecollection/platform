...
from ingests import create_archive_bag_message, send_new_ingest_request


def test_creates_bag_message_without_callback_url(guid):
    bag_url = "s3://example-bukkit/foo/bar.zip"

    resp = create_archive_bag_message(guid=guid, bag_url=bag_url, callback_url=None)
    assert resp == {
...
def test_non_s3_bag_url_is_value_error(guid):
    with pytest.raises(ValueError, match="Unrecognised URL scheme"):
        create_archive_bag_message(
            guid=guid, bag_url="https://example.org", callback_url=None
        )
