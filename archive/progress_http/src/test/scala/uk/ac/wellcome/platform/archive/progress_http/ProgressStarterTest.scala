...
class ProgressStarterTest extends FunSpec with ProgressTrackerFixture with SNS with ProgressGenerators with ScalaFutures with Matchers{
    ...
    withSpecifiedLocalDynamoDbTable(createProgressTrackerTable) { table =>
        withProgressTracker(table) { progressTracker =>
          ...
          whenReady(progressStarter.initialise(progress)) { p =>
            ...
            val requests = listMessagesReceivedFromSNS(topic).map(messageInfo =>
