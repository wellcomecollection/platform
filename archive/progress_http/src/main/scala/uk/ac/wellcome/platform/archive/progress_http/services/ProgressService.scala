...
import akka.http.scaladsl.server.Directives.{complete, get, onSuccess, JavaUUID}
...
class ProgressService(
  ...
  @ApiOperation(
    httpMethod = "GET",
    value = "Returns an ingest request status",
    response = classOf[ResponseDisplayIngest])
  @ApiImplicitParams(
    Array(
  ...
  @ApiResponses(Array(new ApiResponse(code = 404, message = "Not Found Error")))
  ...
  @ApiOperation(
    httpMethod = "POST",
    value = "Initialises an ingest request",
    code = 201,
    response = classOf[ResponseDisplayIngest],
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        name = "ingestRequest",
        required = true,
        dataTypeClass = classOf[RequestDisplayIngest],
        value = "The request to initialise",
        paramType = "body")
    ))
