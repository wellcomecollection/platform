  ...
  describe("Processing callback responses") {
    ...
    val failedStatusCodes =
      Table(
        ...
        (500, "Callback failed for: 12f251b8-c4a9-4afa-85de-c34ec3ed71fe, got 500 Internal Server Error!"),
        (400, "Callback failed for: 12f251b8-c4a9-4afa-85de-c34ec3ed71fe, got 400 Bad Request!")
      )
    it("returns a failed ProgressUpdate when a callback returns with a failed status code") {
      withActorSystem { actorSystem =>
        withMaterializer(actorSystem) { materializer =>
          forAll (failedStatusCodes) { (responseStatus: Int, expectedMsg: String) =>
