...
object DisplayBagManifest {
  def apply(fileManifest: FileManifest): DisplayBagManifest = DisplayBagManifest(fileManifest.checksumAlgorithm.value, fileManifest.files.map(DisplayFileDigest.apply))
}