...
class S3Copier(implicit s3Client: AmazonS3) {
  ...
  private val transferManager = TransferManagerBuilder.standard.withS3Client(s3Client).build // Fixed thread pool
  ...
  def copy(sourceNamespace: String, sourceItemKey: String,
           destinationNamespace: String, destinationItemKey: String): Future[CopyResult] = {
    val copyTransfer =
      transferManager.copy(
        sourceNamespace, sourceItemKey,
