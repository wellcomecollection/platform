...
trait BagIt {
  ...
  def createBag(
    ...
    val tagManifestFiles = dataManifest.toList ++ maybeBagItFile.toList ++ List(
      bagInfoFile)
  ...
  def createValidBagItFile(bagName: BagPath) =
    Some(FileEntry(s"$bagName/bagit.txt", bagItFileContents))

  def dataManifestWithNonExistingFile(bagPath: BagPath,
                                      filesAndDigests: Seq[(String, String)]) =
    Some(
      FileEntry(
  ...
  def dataManifestWithWrongChecksum(bagPath: BagPath,
  ...
  private def createValidManifestFile(bagName: BagPath,
  ...
  def createValidDataManifest(bagName: BagPath,
                              dataFiles: List[(String, String)]) =
    createValidManifestFile(bagName, dataFiles, "manifest-sha256.txt")
  def createValidTagManifest(bagName: BagPath,
                             dataFiles: List[(String, String)]) =
    createValidManifestFile(bagName, dataFiles, "tagmanifest-sha256.txt")
