...
object Progress {
  implicit val statusEncoder = Encoder.instance[Progress.Status](_ match {
    ...
    case CompletedNoCallbackProvided => Json.fromString("completed-callback-none")
    case CompletedCallbackSucceeded => Json.fromString("completed-callback-success")
    case CompletedCallbackFailed => Json.fromString("completed-callback-failure")
  })
  ...
  implicit val licenseDecoder = Decoder.instance[Progress.Status](cursor =>
    for {
      ...
      status match {
        ...
        case "completed-callback-none" => CompletedNoCallbackProvided
        case "completed-callback-success" => CompletedCallbackSucceeded
        case "completed-callback-failure" => CompletedCallbackFailed
      }
    })
