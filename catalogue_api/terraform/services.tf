locals {
  romulus_api_is_pinned   = "${local.pinned_romulus_api == "" ? "false" : "true" }"
  romulus_nginx_is_pinned = "${local.pinned_romulus_api_nginx-delta == "" ? "false" : "true" }"

  remus_api_is_pinned   = "${local.pinned_remus_api == "" ? "false" : "true" }"
  remus_nginx_is_pinned = "${local.pinned_remus_api_nginx-delta == "" ? "false" : "true" }"

  romulus_api_release_id   = "${local.romulus_api_is_pinned == "true" ? local.pinned_romulus_api : var.release_ids["api"]}"
  romulus_nginx_release_id = "${local.romulus_nginx_is_pinned == "true" ? local.pinned_romulus_api_nginx-delta : var.release_ids["nginx_api-delta"]}"

  remus_api_release_id   = "${local.remus_api_is_pinned == "true" ? local.pinned_remus_api : var.release_ids["api"]}"
  remus_nginx_release_id = "${local.remus_nginx_is_pinned == "true" ? local.pinned_remus_api_nginx-delta : var.release_ids["nginx_api-delta"]}"
