...
class ApiV1WorksTest extends ApiWorksTestBase {
  ...
  it("returns a list of works") {
    withApiFixtures(ApiVersions.v1) { case (apiPrefix, indexName, itemType, server: EmbeddedHttpServer) =>
  ...
  it("ignores parameters that are unused when making an API request") {
    withApiFixtures(ApiVersions.v1) { case (apiPrefix, _, _, server: EmbeddedHttpServer) =>
      server.httpGet(
  ...
  it("searches different indices with the ?_index query parameter") {
    withApiFixtures(ApiVersions.v1) { case (apiPrefix, indexName, itemType, server: EmbeddedHttpServer) =>
      ...
      insertIntoElasticsearch(indexName = "alt_records", itemType = itemType, work_alt)
