...
class ApiV1WorksTest extends ApiWorksTestBase {
  ...
  it("returns a list of works") {
    withApiFixtures(apiVersion = "v1") { case (apiPrefix, indexName, itemType, server: EmbeddedHttpServer) =>
  ...
  it("ignores parameters that are unused when making an API request") {
    withApiFixtures(apiVersion = "v1") { case (apiPrefix, _, _, server: EmbeddedHttpServer) =>
      server.httpGet(
