...
def test_complete_ec2_shutdown_if_no_ecs_cluster(
    lifecycle_hook_name, autoscaling_group_name, ec2_instance_id, ec2_terminating_event
):
    ...
    mocked_asg_client.complete_lifecycle_action.assert_called_once_with(
...
def test_drain_ecs_instance_if_running_tasks(
    ...
    mocked_asg_client.record_lifecycle_action_heartbeat.assert_called_once_with(
        LifecycleHookName=lifecycle_hook_name,
        AutoScalingGroupName=autoscaling_group_name,
    ...
    mocked_asg_client.complete_lifecycle_action.assert_not_called()

    messages = fake_sqs_client.receive_message(
        QueueUrl=moto_queue_url, MaxNumberOfMessages=1
    )
    message_body = messages["Messages"][0]["Body"]
    inner_message = json.loads(message_body)["Message"]

    assert json.loads(inner_message)["default"] == json.dumps(ec2_terminating_message)
