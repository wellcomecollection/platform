...
@pytest.fixture
def ecs_container_instance_arn(ec2_instance_id, ecs_cluster_name, ecs_cluster_arn):
    ecs_client = boto3.client("ecs")
    ...
    resp = ecs_client.register_container_instance(
        cluster=ecs_cluster_name, instanceIdentityDocument=instance_id_document
    )
    ...
    ec2_client.create_tags(
        ...
        Tags=[
            {"Key": "clusterArn", "Value": ecs_cluster_arn},
            {"Key": "containerInstanceArn", "Value": instance_arn},
        ],
    )
...
def ecs_task(ecs_cluster_name, ecs_cluster_arn, ecs_container_instance_arn):
    ...
    tasks = fake_ecs_client.list_tasks(
        cluster=ecs_cluster_arn, containerInstance=ecs_container_instance_arn
    )
...
def ec2_terminating_message(
    lifecycle_hook_name, autoscaling_group_name, ec2_instance_id, lifecycle_action_token
):
    return {
