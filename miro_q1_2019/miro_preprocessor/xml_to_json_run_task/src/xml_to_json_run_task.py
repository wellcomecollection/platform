...
def command_for_xml_to_json_task(event, cluster_name, container_name, task_definition):
    ...
    cmd = [f"--bucket={bucket_name}", f"--src={object_src}", f"--dst={object_dst}"]
...
def main(event, _ctxt=None, sns_client=None):
    ...
    task_commands = [
        command_for_xml_to_json_task(
            event, cluster_name, container_name, task_definition
        )
    ...
    for cmd in task_commands:
        sns_utils.publish_sns_message(
            sns_client, topic_arn, cmd, subject="xml_to_json_run_task"
        )
