...
def _put_object(s3_client, object_body, key):
    s3_client.put_object(Bucket=bucket_name, Key=key, Body=object_body)


def _put_metadata_json(s3_client, metadata):
    s3_client.put_object(
        Bucket=bucket_name, Key="metadata.json", Body=json.dumps(metadata)
    )
...
def _get_msg(sqs_client, queue_url):
    ...
    return {
        "subject": subject,
        "message": json.loads(json.loads(inner_message)["default"]),
    }
...
def test_fetch_json_s3_data():
    ...
    _setup(s3_client=s3_client, metadata=metadata)
...
def test_image_sorter_catalogue_api(image_sorter_sns_sqs, s3_put_event):
    ...
    _setup(s3_client=s3_client, metadata=metadata, sns_sqs=sns_sqs)
...
def test_image_sorter_id_exceptions(image_sorter_sns_sqs, s3_put_event):
    ...
    miro_id = "V1234567"
    metadata = collection_image_data(image_no_calc=miro_id)

    id_exceptions_csv_body = (
        f"""miro_id,cold_store,tandem_vault,catalogue_api\n{miro_id},true,,"""
    )
...
def test_image_sorter_contrib_exceptions_match(image_sorter_sns_sqs, s3_put_event):
    ...
    metadata = collection_image_data(image_source_code="FOO", collection="images-A")
...
def test_image_sorter_contrib_exceptions_no_match(image_sorter_sns_sqs, s3_put_event):
    ...
    metadata = collection_image_data(image_source_code="BAR", collection="images-A")
