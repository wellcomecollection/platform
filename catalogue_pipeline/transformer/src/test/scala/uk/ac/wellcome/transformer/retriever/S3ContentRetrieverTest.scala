...
class S3ContentRetrieverTest extends FunSpec with Matchers with S3Local with ScalaFutures{
  ...
  it("fails with GracefulFailureException if the key does not exist in the bucket") {
