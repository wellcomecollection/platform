...
import uk.ac.wellcome.platform.matcher.models.{IdentifierList, LinkedWork, LinkedWorksGraph, LinkedWorksIdentifiersList}
...
class LinkedWorkMatcher {
  ...
  private def matchLinkedWorks(work: UnidentifiedWork): List[IdentifierList] = {
    ...
    updatedLinkedWorkGraph.linkedWorksList.groupBy(_.setId).map {
