...
object RedirectFinder {
  ...
  def redirects(work: WorkUpdate,
                existingRedirects: List[Redirect]): List[Redirect] = {
    val directlyAffectedExistingRedirects = existingRedirects.filter(
      redirect => work.linkedIds.contains(redirect.source))
  ...
  private def newRedirects(linkedIds: List[String],
    ...
    linkedIds.map(linkedId =>
      Redirect(source = linkedId, target = combinedIdentifier))
  }

  private def redirectToCombined(existingRedirects: List[Redirect],
                                 combinedIdentifier: String) = {
    existingRedirects.map(_.copy(target = combinedIdentifier))
  }

  private def combinedIdentifier(linkedIds: List[String],
                                 directlyAffectedRedirects: List[Redirect]) = {
