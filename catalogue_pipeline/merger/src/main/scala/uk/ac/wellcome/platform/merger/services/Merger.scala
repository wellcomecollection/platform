...
class Merger(rules: List[MergerRule]) {
  def merge(works: Seq[UnidentifiedWork]): Seq[BaseWork] = {
    rules.foldLeft(works: Seq[BaseWork])((works, rule) => rule.mergeAndRedirectWorks(works))
  }
}

object PlatformMerger extends Merger(List(SierraPhysicalDigitalMergeRule, SierraMiroMergeRule))
