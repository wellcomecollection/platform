            ...
            withLocalSqsQueueAndDlq {
              case QueuePair(queue, dlq) =>
                withServer(queue, topic, bucket, table) { _ =>
                  ...
                  val result =
                    vhs.updateRecord(recorderWorkEntry.id)(ifNotExisting =
                      (recorderWorkEntry, EmptyMetadata()))((_, _) =>
                      throw new RuntimeException(
                        "Not possible, VHS is empty!"))
                  whenReady(result) { _ =>
                    val matcherResult = MatcherResult(
                      Set(MatchedIdentifiers(Set(WorkIdentifier(
                        identifier = recorderWorkEntry.id,
                        version = 1)))))
