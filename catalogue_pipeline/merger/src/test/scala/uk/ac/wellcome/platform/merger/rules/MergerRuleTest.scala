...
class MergerRuleTest extends FunSpec with WorksGenerators with Matchers {
  ...
  it("returns the works unchanged if the list can be partitioned but the work pair merger returns none") {
    val mergerRule = new MergerRule with Partitioner with WorkPairMerger {
      override protected def partitionWorks(
                                             works: Seq[BaseWork])
      : Option[
        Partition] = Some(Partition(works.head.asInstanceOf[UnidentifiedWork], works.tail.head.asInstanceOf[UnidentifiedWork], works.tail.tail))
      override protected def mergeAndRedirectWorkPair(
                                                       firstWork: UnidentifiedWork,
                                                       secondWork: UnidentifiedWork)
      : Option[
  ...
  it("increments the version of merged/redirected works") {
    val mergerRule = new MergerRule with Partitioner with WorkPairMerger {
      ...
      override protected def mergeAndRedirectWorkPair(
                                                       ...
                                                       secondWork: UnidentifiedWork)
      : Option[
        MergedWork] = Some(MergedWork(firstWork, UnidentifiedRedirectedWork(source = secondWork, target = firstWork)))
    }
