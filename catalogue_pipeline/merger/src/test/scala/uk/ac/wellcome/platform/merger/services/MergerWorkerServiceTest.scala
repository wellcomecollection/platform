...
class MergerWorkerServiceTest
      ...
      case (QueuePair(queue, dlq), _) =>
        val matcherResult = MatcherResult(
          Set(MatchedIdentifiers(
            Set(WorkIdentifier(identifier = "sierra/b123456", version = 1)))))
  ...
  def withMergerWorkerServiceFixtures[R](
    testWith: TestWith[(QueuePair, MetricsSender), R]) = {
  ...
  def withMergerWorkerService[R](actorSystem: ActorSystem,
