...
class SierraMiroMergeRuleTest extends FunSpec with Matchers with WorksGenerators {
  ...
  describe("Merges a single-page Miro work into a Sierra work") {
    ...
    it("does not duplicate sierra-system-number when merging miro items") {
      ...
      val miroWork = createMiroWorkWith(otherIdentifiers = List(libraryReferenceIdentifier, sierraWork.sourceIdentifier))
      ...
      val expectedMergedWork = sierraWork.copy(
        otherIdentifiers = sierraWork.otherIdentifiers ++ List(miroWork.sourceIdentifier, libraryReferenceIdentifier),
        items = List(sierraItem.copy(
          agent = sierraItem.agent.copy(
            locations =
              sierraItem.agent.locations ++ miroWork.items.head.agent.locations
    ...
    it("does not merge Miro DigitalLocation if the Sierra work already has a DigitalLocation") {
      val sierraItem = createIdentifiableItemWith(locations = List(createDigitalLocation))
      val sierraWork = createSierraWorkWith(items = List(sierraItem))
      val miroWork = createMiroWork
