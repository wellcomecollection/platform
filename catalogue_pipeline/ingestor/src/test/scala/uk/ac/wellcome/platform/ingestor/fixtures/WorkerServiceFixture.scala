...
trait WorkerServiceFixture extends ElasticsearchFixtures with Messaging { this: Suite =>
  def withWorkerService[R](queue: Queue, indexName: String, elasticClient: HttpClient = elasticClient)(testWith: TestWith[IngestorWorkerService, R]): R =
    withActorSystem { actorSystem =>
      withMetricsSender(actorSystem) { metricsSender =>
        withMessageStream[IdentifiedBaseWork, R](actorSystem, queue, metricsSender) { messageStream =>
