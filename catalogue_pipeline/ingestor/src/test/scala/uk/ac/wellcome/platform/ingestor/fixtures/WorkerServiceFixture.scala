...
trait WorkerServiceFixture extends ElasticsearchFixtures with Messaging { this: Suite =>
  val documentType = "work"

  def withWorkerService[R](bucket: Bucket, queue: Queue, indexName: String, elasticClient: HttpClient = elasticClient)(testWith: TestWith[IngestorWorkerService, R]): R =
    withActorSystem { actorSystem =>
      withMetricsSender(actorSystem) { metricsSender =>
        withMessageStream[IdentifiedBaseWork, R](actorSystem, bucket, queue, metricsSender) { messageStream =>
