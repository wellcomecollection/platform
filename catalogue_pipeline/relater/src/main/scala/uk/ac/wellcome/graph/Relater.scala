...
class Relater(graph: GraphTraversalSource) {
  ...
  def getConnectedGraph(str: String): List[String] = {
    val list = graph.V().hasLabel(str).emit().repeat(graph.V().both("same-as")).until(graph.V().cyclicPath()).label().toSet.toList
    list
  }
  ...
  def updateNode(node: Node): Unit = {
    ...
    node.connectedTo.foreach {destLabel =>
      ...
      val maybeEdge = graph.V(startNode.id).out("same-as").hasLabel(destLabel).tryNext()

      if(maybeEdge.isEmpty){
        graph.V(startNode.id).as("a").V(destNode.id).as("b").addE("same-as").from("a").to("b").iterate()
      }
