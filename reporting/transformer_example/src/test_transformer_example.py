...
def create_sns_message(bucket_name, id, key):
    return {'Records': [{
        ...
        'EventVersion': '1.0',
        'Sns': {'Message': f"{{\"id\":\"{id}\",\"version\":2,\"location\":{{\"namespace\":\"{bucket_name}\",\"key\":\"{key}\"}}}}",
                'MessageAttributes': {},
...
def test(s3_client, bucket, elasticsearch_client, elasticsearch_index):
    ...
    transformer_example.main(event, {}, s3_client, elasticsearch_client, elasticsearch_index, elasticsearch_doctype)
