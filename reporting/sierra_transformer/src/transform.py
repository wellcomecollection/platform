...
def transform(input_data):
    ...
    bib_record["bibLevel_value"] = unpack(
        record=bib_record, field_name="bibLevel", subfields_to_keep="value"
    )
    try:
    ...
    bib_record["country_name"] = unpack(
        record=bib_record, field_name="country", subfields_to_keep="name"
    )
    try:
    ...
    bib_record["lang_name"] = unpack(
        record=bib_record, field_name="lang", subfields_to_keep="name"
    )
    try:
        del bib_record["lang"]
    except KeyError:
    ...
    bib_record["materialType_code"] = unpack(
        record=bib_record, field_name="materialType", subfields_to_keep="code"
    )
    try:
    ...
    locations_name, locations_code = unpack(
        record=bib_record, field_name="locations", subfields_to_keep=["name", "code"]
    )
    ...
    # unpack orders
    orders_date = unpack(
        record=bib_record, field_name="orders", subfields_to_keep="date"
    )
    try:
    ...
    try:
        ...
        order_location_codes, order_location_names = unpack(
            record=orders, field_name="location", subfields_to_keep=["name", "code"]
        )
