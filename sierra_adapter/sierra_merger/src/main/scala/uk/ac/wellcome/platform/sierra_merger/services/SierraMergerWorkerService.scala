...
class SierraMergerWorkerService @Inject()(
  reader: SQSReader,
  system: ActorSystem,
  metrics: MetricsSender,
  sierraBibMergerUpdaterService: SierraMergerUpdaterService,
  ...
  override def processMessage(message: SQSMessage): Future[Unit] =
    resourceType match {
      case "bibs" => JsonUtil.fromJson[SierraBibRecord](message.body) match {
        case Success(record) => sierraBibMergerUpdaterService.updateBib(record)
