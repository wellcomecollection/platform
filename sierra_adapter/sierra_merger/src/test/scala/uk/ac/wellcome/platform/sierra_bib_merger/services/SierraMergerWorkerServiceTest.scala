package uk.ac.wellcome.platform.sierra_merger.services
...
import uk.ac.wellcome.platform.sierra_merger.Server
import uk.ac.wellcome.platform.sierra_merger.locals.DynamoDBLocal
import uk.ac.wellcome.test.utils.{AmazonCloudWatchFlag, SQSLocal}
import uk.ac.wellcome.utils.JsonUtil

import uk.ac.wellcome.models.SierraBibRecord._

class SierraMergerWorkerServiceTest
    extends FunSpec
  ...
  private def sendBibRecordToSQS(record: SierraBibRecord) = {
    ...
    val message = SQSMessage(
      subject = Some("Test message sent by SierraMergerWorkerServiceTest"),
      body = messageBody,
      topic = "topic",
