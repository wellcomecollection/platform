...
object SequentialS3Sink extends Logging {
  def apply(client: AmazonS3, bucketName: String, keyPrefix: String = "")(
    implicit executionContext: ExecutionContext): Sink[(Json, Long), Future[Done]] = {
    Sink.foreachParallel(10) {
      case (json: Json, index: Long) => {
        // Zero-pad the index to four digits for easy sorting, e.g. "1" ~> "0001", "25" ~> "0025"
        val key = f"${keyPrefix}${index}%04d.json"
