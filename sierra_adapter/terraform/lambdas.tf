...
module "lambda_sierra_items_merger_filter" {
  source = "git::https://github.com/wellcometrust/terraform.git//lambda?ref=v1.2.0"

  name        = "sierra_items_merger_filter_filter"
  module_name = "transformer_sns_filter"
  description = "Filters DynamoDB events for the Sierra items merger"
  timeout     = 6

  environment_variables = {
    TOPIC_ARN = "${module.sierra_items_merger_events_topic.arn}"
  }
...
module "trigger_sierra_items_merger_filter" {
  source = "git::https://github.com/wellcometrust/terraform.git//lambda/trigger_sns?ref=v1.0.0"

  sns_trigger_arn      = "${module.sierra_to_dynamo_items.topic_arn}"
  lambda_function_arn  = "${module.lambda_sierra_items_merger_filter.arn}"
  lambda_function_name = "${module.lambda_sierra_items_merger_filter.role_name}"
}
