...
object BibMerger {
  ...
  /** Return the most up-to-date combination of the merged record and the
    ...
    */
  def mergeBibRecord(mergedSierraRecord: MergedSierraRecord, sierraBibRecord: SierraBibRecord): MergedSierraRecord = {
    ...
    val isNewerData = mergedSierraRecord.maybeBibData match {
      case Some(bibData) => sierraBibRecord.modifiedDate.isAfter(bibData.modifiedDate)
      case None => true
    }
