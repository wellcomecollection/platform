...
object AkkaS3Builder extends AWSClientConfigBuilder with Logging {
  def buildAkkaS3Client(actorSystem: ActorSystem, awsClientConfig: AWSClientConfig): S3Client = {
    val regionProvider =
    ...
    val endpointUrl = awsClientConfig.endpoint
  ...
  def buildAkkaS3Client(config: Config): S3Client =
    buildAkkaS3Client(
